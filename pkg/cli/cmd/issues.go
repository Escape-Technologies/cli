...
INFO        PROTOCOL    Misconfigured Cache Control Header    OPEN      https://assets.com        2025-06-27T06:02:18.874Z    https://app.escape.tech/all-risks/issues/00000000-0000-0000-0000-000000000001/overview/`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		result = append(result, fmt.Sprintf("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s", issue.GetSeverity(), issue.GetCategory(), issue.GetName(), issue.GetStatus(), issue.GetAsset().Name, issue.GetCreatedAt(), issue.GetLinks().IssueOverview, issue.GetLinks().IssueOverview))
